...
The type system ensures each suffix in the chain receives the right element type. A `Lista-Szám` can use `-felett`; a `Lista-Szöveg` cannot.

---

## Suffix reference

| Suffix | Arg | Produces | Description |
|---|---|---|---|
| `-rendezve` | — | `Lista-T` | Sort ascending |
| `-fordítva` | — | `Lista-T` | Reverse |
| `-egyedi` | — | `Lista-T` | Remove duplicates |
| `-első` | — | `T` | First element |
| `-utolsó` | — | `T` | Last element |
| `-hossz` | — | `Szám` | Length |
| `-hozzáad` | element | `Lista-T` | Append element |
| `-eltávolít` | element | `Lista-T` | Remove first occurrence |
| `-lapítva` | — | `Lista-T` | Flatten one level |
